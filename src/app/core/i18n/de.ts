export const de = {
  nav: {
    overview: 'Übersicht',
    collection: 'Sammlung',
    decks: 'Decks',
    wishlist: 'Wishlist',
    logout: 'Logout',
  },
  games: {
    mtg: 'Magic: The Gathering',
    yugioh: 'Yu-Gi-Oh!',
    pokemon: 'Pokémon',
    comingSoon: 'bald verfügbar',
  },
  footer: {
    cardDataFrom: 'Kartendaten von',
    impressum: 'Impressum',
  },
  common: {
    card: 'Karte',
    cards: 'Karten',
    unknown: 'Unbekannt',
    yes: 'Ja',
    no: 'Nein',
    forDeck: 'Für {{name}}',
  },
  colors: {
    white: 'Weiß',
    blue: 'Blau',
    black: 'Schwarz',
    red: 'Rot',
    green: 'Grün',
    multicolor: 'Mehrfarbig',
    colorless: 'Farblos',
    special: 'Specials',
  },
  rarity: {
    common: 'Gewöhnlich',
    uncommon: 'Ungewöhnlich',
    rare: 'Selten',
    mythic: 'Mythisch',
    special: 'Speziell',
    bonus: 'Bonus',
  },
  auth: {
    loginTitle: 'Anmelden',
    email: 'E-Mail',
    password: 'Passwort',
    emailInvalid: 'Bitte eine gültige E-Mail-Adresse eingeben.',
    passwordMinLength: 'Mindestens 6 Zeichen.',
    loginSubmit: 'Anmelden',
    google: 'Mit Google anmelden',
    noAccount: 'Noch kein Konto?',
    inviteLink: 'Einladungscode einlösen',
    or: 'oder',
    tryDemo: 'Demo ausprobieren',
    demoHint: 'Ohne Registrierung testen — kein eigenes Konto nötig. Läuft nach 24 Stunden automatisch ab.',
    demoSeeding: 'Demo wird eingerichtet…',
    demoSeedingHint: 'Sammlung, Decks und Wunschliste werden befüllt — das dauert einen Moment.',
    loginFailed: 'Login fehlgeschlagen.',
    googleFailed: 'Google-Login fehlgeschlagen.',
    demoFailed: 'Demo-Login fehlgeschlagen.',
    inviteTitle: 'Einladungscode einlösen',
    inviteCodeLabel: 'Einladungscode',
    inviteCodePlaceholder: 'Code eingeben',
    inviteInvalid: 'Ungültiger oder abgelaufener Einladungscode.',
    inviteSubmit: 'Konto erstellen',
    inviteGoogle: 'Mit Google registrieren',
    inviteRequiredError: 'Für die Registrierung wird ein gültiger Einladungslink benötigt.',
    demoExpired: 'Deine Demo ist abgelaufen. Bitte melde dich erneut an oder starte eine neue Demo.',
    demoBanner: 'Demo endet in {{hours}} Stunde(n)',
    confirmPassword: 'Passwort bestätigen',
    passwordMismatch: 'Passwörter stimmen nicht überein.',
    haveAccount: 'Schon ein Konto?',
    loginLink: 'Anmelden',
    accountCreated: 'Konto erstellt. Du kannst dich jetzt anmelden.',
    completing: 'Anmeldung wird abgeschlossen…',
  },
  dashboard: {
    welcome: 'Willkommen bei TCG Vault',
    loading: 'Lade Sammlung…',
    loadError: 'Sammlung konnte nicht geladen werden.',
    empty: 'Noch keine Karten in deiner Sammlung.',
    colorDistribution: 'Farbverteilung',
    attributeDistribution: 'Attributverteilung',
    priceDistribution: 'Preisverteilung',
    popularCards: 'Beliebte Karten',
    saltiestCards: 'Salzigste Karten',
    saltiestCardsSubtitle: 'Karten die deine Mitspieler am meisten nerven',
    colorCategories: 'Farbkategorien',
    attributeCategories: 'Attributkategorien',
    typeDistribution: 'Typverteilung',
    typeCategories: 'Typkategorien',
    noCards: 'Keine Karten',
  },
  collection: {
    title: 'Sammlung',
    loading: 'Lade Sammlung…',
    loadError: 'Sammlung konnte nicht geladen werden.',
    searchAndFilter: 'Suche & Filter',
    searchPlaceholder: 'Karte in der Sammlung suchen…',
    addCardAria: 'Karte hinzufügen',
    scanCardAria: 'Karte per Kamera scannen',
    scanCardDemoAria: 'Scanner (im Demo-Modus gesperrt)',
    demoBadge: 'Demo',
    all: 'Alle',
    manaCurve: 'Mana-Kurve',
    emptyCollection: 'Noch keine Karten in deiner Sammlung. Füge deine erste Karte hinzu!',
    noResults: 'Keine Karten gefunden.',
    oracleBackfillDone: 'Sammlung wurde aktualisiert',
  },
  addCard: {
    title: 'Karte hinzufügen',
    searchPlaceholder: 'Kartenname suchen…',
    searching: 'Suche…',
    searchFailed: 'Suche fehlgeschlagen.',
    noCardsFound: 'Keine Karten gefunden.',
    backToSearch: '← Zurück zur Suche',
    pickPrint: '{{name}} – Druck wählen',
    loadingPrints: 'Lade Drucke…',
    printsFailed: 'Drucke konnten nicht geladen werden.',
    backToPrints: '← Andere Version wählen',
    quantity: 'Menge',
    foil: 'Foil',
    condition: 'Zustand',
    submit: 'Hinzufügen',
    addFailed: 'Karte konnte nicht hinzugefügt werden.',
  },
  cardDetail: {
    title: 'Kartendetails',
    quantity: 'Menge',
    set: 'Set',
    rarity: 'Seltenheit',
    foil: 'Foil',
    finish: 'Ausführung',
    finish_halo: 'Halo',
    finish_foil: 'Foil',
    finish_etched: 'Etched',
    price: 'Preis',
    cardmarketLink: 'Auf Cardmarket ansehen',
    editLink: 'Menge / Foil bearbeiten',
    saveButton: 'Speichern',
    saveFailed: 'Änderungen konnten nicht gespeichert werden.',
    invalidQuantity: 'Menge muss mindestens 1 sein.',
    deleteLink: 'Aus Sammlung entfernen',
    confirmDeleteText: 'Karte wirklich aus der Sammlung entfernen?',
    confirmDeleteButton: 'Ja, entfernen',
    cancel: 'Abbrechen',
    deleteFailed: 'Karte konnte nicht entfernt werden.',
  },
  decks: {
    title: 'Decks',
    bannerAria: 'Precon-Decks durchsuchen',
    bannerText: 'Precon-Decks durchsuchen & hinzufügen',
    commanderRecsButton: 'Commander-Empfehlungen aus deiner Sammlung',
    formatRecsButton: 'Format-Deck-Empfehlungen',
    almostComplete: 'Fast komplett',
    almostCompleteHint: 'Precon-Decks, von denen du bereits mindestens 75 % der Karten besitzt.',
    matchInCollection: '{{percent}}% in Sammlung',
    addToMyDecks: 'Zu meinen Decks hinzufügen',
    missingCardsToWishlist: '{{count}} fehlende Karten auf Wunschliste',
    cardsCount: '{{count}} Karten',
    noCards: 'Keine Karten',
    loading: 'Lade Decks…',
    loadError: 'Decks konnten nicht geladen werden.',
    empty: 'Noch keine Decks. Über das Banner ein Precon-Deck hinzufügen!',
    emptyNoPrecon: 'Noch keine Decks. Über „Deck importieren" eine Kartenliste einfügen!',
    addRecommendationFailed: 'Deck konnte nicht hinzugefügt werden.',
    wishlistUpdateFailed: 'Wunschliste konnte nicht aktualisiert werden.',
  },
  browseDecks: {
    title: 'Precon-Deck hinzufügen',
    searchPlaceholder: 'Deck-Name suchen…',
    loadingList: 'Lade Deck-Liste…',
    listFailed: 'Deck-Liste konnte nicht geladen werden.',
    noDecksFound: 'Keine Decks gefunden.',
    searchHint: 'Nach Deck-Name oder einer enthaltenen Karte/Charakter suchen, z. B. „Sephiroth“ oder „Commander“.',
    searchHintYugioh: 'Nach Deck-Name oder einer enthaltenen Karte suchen, z. B. „Dark Magician“ oder „Dragon“.',
    indexing: 'Kartennamen werden im Hintergrund indiziert ({{progress}})…',
    backToSearch: '← Zurück zur Suche',
    loadingDetail: 'Lade Kartenliste…',
    detailFailed: 'Deck konnte nicht geladen werden.',
    cardsCount: '{{count}} verschiedene Karten',
    skippedOne: '1 Karte konnte nicht zugeordnet werden.',
    skippedOther: '{{count}} Karten konnten nicht zugeordnet werden.',
    submit: 'Zu meinen Decks hinzufügen',
    addFailed: 'Deck konnte nicht hinzugefügt werden.',
    matchModeLabel: 'Verplante Karten zählen',
    matchModeStrict: 'Strikt',
    matchModeFlexible: 'Flexibel',
  },
  deckDetail: {
    missingCards: '{{count}} Karten fehlen noch in der Sammlung.',
    onWishlist: 'Auf der Wunschliste ✓',
    wishlistButton: 'Fehlende Karten auf Wunschliste',
    containedCards: 'Enthaltene Karten',
    deleteLink: 'Deck entfernen',
    confirmDeleteText: 'Deck wirklich entfernen? Die Karten bleiben in deiner Sammlung.',
    confirmDeleteButton: 'Ja, entfernen',
    cancel: 'Abbrechen',
    deleteFailed: 'Deck konnte nicht entfernt werden.',
    wishlistFailed: 'Wunschliste konnte nicht aktualisiert werden.',
    plannedElsewhereHint: '(davon {{percent}}% in anderen Decks verplant)',
    bindingDetailsTitle: 'Details',
    substituteNote: 'Ersetzt {{name}} ({{print}})',
    substituteOwnedNote: 'deine Version: {{name}} aus {{print}}',
    assignedElsewhereNote: 'Bereits in „{{deckName}}" verplant',
    availableNote: 'Verfügbar: {{available}} von {{needed}}',
    releaseButton: '🔓 Freigeben',
    confirmReleaseText: '{{cardName}} aus „{{deckName}}" freigeben? Das Deck wird dadurch unvollständig.',
    confirmReleaseButton: 'Freigeben',
    releaseFailed: 'Karte konnte nicht freigegeben werden.',
  },
  commanderRecs: {
    title: 'Commander-Empfehlungen',
    hint: 'Jede Legendary Creature in deiner Sammlung wird als Commander geprüft, plus Commander, die du noch nicht besitzt, deren EDHREC-Durchschnittsdeck aber schon stark durch deine übrigen Karten abgedeckt ist.',
    scanningCards: 'Durchsuche Sammlung nach möglichen Commandern… ({{progress}})',
    checkingCommanders: 'Prüfe Commander… ({{progress}})',
    noResults: 'Keine passenden Commander gefunden.',
    cardsCount: '{{matched}} / {{total}} Karten',
    freeMatchBadge: '{{percent}}% frei',
    totalMatchBadge: '{{percent}}% gesamt',
    missingTag: 'Commander fehlt in Sammlung',
    loadFailed: 'Empfehlungen konnten nicht geladen werden.',
    backToList: '← Zurück zur Liste',
    ownedSection: 'Frei verfügbar ({{count}})',
    plannedElsewhereSection: 'In anderen Decks verplant ({{count}})',
    missingSection: 'Noch fehlend ({{count}})',
    detailFailed: 'Kartenliste konnte nicht geladen werden.',
    addDeckFailed: 'Deck konnte nicht hinzugefügt werden.',
    deckAdded: 'Zur Deckliste hinzugefügt.',
  },
  formatRecs: {
    title: 'Format-Deck-Empfehlungen',
    hint: 'Wähle ein Format - beliebte Archetypen werden gegen deine Sammlung geprüft, inklusive was schon in anderen Decks verplant ist.',
    formatStandard: 'Standard',
    formatPioneer: 'Pioneer',
    formatModern: 'Modern',
    formatLegacy: 'Legacy',
    formatVintage: 'Vintage',
    formatPauper: 'Pauper',
    backToFormats: '← Anderes Format',
    backToList: '← Zurück zur Liste',
    scanningHubs: 'Suche Archetypen…',
    checkingHubs: 'Prüfe Archetypen… ({{progress}})',
    noResults: 'Keine passenden Archetypen gefunden.',
    loadFailed: 'Empfehlungen konnten nicht geladen werden.',
    detailFailed: 'Kartenliste konnte nicht geladen werden.',
    addDeckFailed: 'Deck konnte nicht hinzugefügt werden.',
  },
  wishlist: {
    title: 'Wunschliste',
    addAria: 'Karte hinzufügen',
    loading: 'Lade Wunschliste…',
    loadError: 'Wunschliste konnte nicht geladen werden.',
    empty: 'Noch keine Karten auf der Wunschliste.',
    totalValue: 'Gesamtwert: {{value}}',
    cardmarketLink: 'Auf Cardmarket ansehen',
    priorityLow: 'Niedrig',
    priorityMid: 'Mittel',
    priorityHigh: 'Hoch',
    notesPlaceholder: 'Notiz…',
    removeAria: 'Entfernen',
  },
  scanner: {
    close: 'Schließen',
    starting: 'Kamera wird gestartet…',
    bulkDataLoading: 'Kartendatenbank wird geladen... {{percent}}%',
    cameraPermissionDenied: 'Kamerazugriff verweigert. Bitte in den Browser-Einstellungen erlauben.',
    noCameraFound: 'Keine Kamera gefunden.',
    backToCollection: '← Zurück zur Sammlung',
    cameraSelect: 'Kamera auswählen',
    focusSlider: 'Fokus',
    cameraFallbackLabel: 'Kamera {{index}}',
    chooseCardHintMedium: 'Welche Karte ist das?',
    chooseCardHintLow: 'Bitte Karte auswählen',
    noneOfThese: 'Keine davon — weiter scannen',
    addToCollection: 'Zur Collection hinzufügen',
    keepScanning: 'Weiter scannen',
    autoAddHint: 'Wird gleich automatisch hinzugefügt…',
    addedToast: '✓ {{name}} zur Sammlung hinzugefügt',
    undoLastAdd: '{{name}} aus der Sammlung entfernen',
    undoToast: '↩ {{name}} entfernt',
    undoFailed: 'Konnte nicht rückgängig gemacht werden.',
    notRecognized: 'Karte nicht erkannt — bitte Karte deutlicher vor die Kamera halten',
    rateLimited: 'Zu viele Anfragen — kurze Pause…',
    addFailed: 'Karte konnte nicht hinzugefügt werden.',
    limitTitle: '⚠️ Tageslimit erreicht',
    limitMessage: 'Das tägliche Scan-Limit für die KI-Erkennung ist erschöpft. Der Scanner wechselt automatisch auf den Basis-Modus. Das Limit wird um Mitternacht zurückgesetzt.',
    noGeminiKeyHint: 'Für KI-Erkennung bitte Gemini API Key in den Einstellungen hinterlegen',
    geminiErrorTitle: '⚠️ Gemini-Fehler',
    geminiErrorFallback: 'Gemini-Erkennung fehlgeschlagen — Details siehe Konsole',
    goToSettings: 'Zu den Einstellungen',
    tesseractFallbackTitle: '📝 Basis-Modus aktiv',
    tesseractFallbackMessage:
      'Gemini war mehrfach nicht erreichbar — der Scanner nutzt vorübergehend die lokale Texterkennung.',
  },
  demoScanBlocked: {
    title: 'Scanner im Demo-Modus gesperrt',
    message: 'Der Scanner ist im Demo-Modus nicht verfügbar. Bitte registriere dich für vollen Zugriff.',
    cta: 'Jetzt registrieren',
    close: 'Schließen',
  },
  addWishlist: {
    title: 'Zur Wunschliste hinzufügen',
    priorityLabel: 'Priorität',
    notesLabel: 'Notizen',
    notesPlaceholder: 'Optional…',
    submit: 'Zur Wunschliste hinzufügen',
    addFailed: 'Karte konnte nicht zur Wunschliste hinzugefügt werden.',
  },
  yugioh: {
    attributeFire: 'FEUER',
    attributeWater: 'WASSER',
    attributeEarth: 'ERDE',
    attributeWind: 'WIND',
    attributeLight: 'LICHT',
    attributeDark: 'DUNKEL',
    spellTrap: 'Zauber/Falle',
    attack: 'ATK',
    defense: 'DEF',
    cardType: 'Kartentyp',
    attribute: 'Attribut',
    banlistStatus: 'Banlist-Status',
    banlistForbidden: 'Verboten',
    banlistLimited: 'Limitiert',
    banlistSemiLimited: 'Semi-limitiert',
  },
  pokemon: {
    typeGrass: 'Pflanze',
    typeFire: 'Feuer',
    typeWater: 'Wasser',
    typeLightning: 'Elektro',
    typePsychic: 'Psycho',
    typeFighting: 'Kampf',
    typeDarkness: 'Finsternis',
    typeMetal: 'Metall',
    typeFairy: 'Fee',
    typeDragon: 'Drache',
    typeColorless: 'Farblos',
    supertypeTrainer: 'Trainer',
    supertypeEnergy: 'Energie',
    supertype: 'Kartentyp',
  },
  selectGame: {
    title: 'Wähle dein Spiel',
    subtitle: 'Du kannst jederzeit im Header wechseln',
    cardsInCollection: '{{count}} Karten in Sammlung',
    moreGames: 'Weiteres Spiel',
    chooseFailed: 'Spiel konnte nicht gewählt werden.',
  },
  deckImport: {
    openButton: '📋 Deck importieren',
    title: 'Deck importieren',
    nameLabel: 'Deck-Name',
    defaultName: 'Importiertes Deck',
    listLabel: 'Kartenliste',
    listPlaceholder: '1 Aang, Air Nomad (TLE) 234\n1 Katara, Heroic Healer\n8 Plains',
    importButton: 'Importieren',
    cancelButton: 'Abbrechen',
    progress: 'Importiere Karte {{current}} von {{total}}…',
    doneSummary: '✓ {{imported}} Karten importiert, {{failed}} nicht gefunden',
    doneSummaryAllOk: '✓ {{imported}} Karten importiert',
    failedListHeading: 'Nicht gefunden:',
    doneButton: 'Fertig',
    importFailed: 'Deck konnte nicht importiert werden.',
  },
  archetypeBrowser: {
    openButton: 'Archetypen durchsuchen',
    title: 'Archetypen durchsuchen',
    hint: 'Zeigt alle Karten eines Themas (Archetyp) — kein kuratiertes Deck, sondern der gesamte Kartenpool zum Stöbern.',
    searchPlaceholder: 'Archetyp suchen, z. B. „Blue-Eyes“…',
    loadingList: 'Lade Archetyp-Liste…',
    listFailed: 'Archetyp-Liste konnte nicht geladen werden.',
    noArchetypesFound: 'Keine Archetypen gefunden.',
    backToSearch: '← Zurück zur Suche',
    loadingCards: 'Lade Karten…',
    cardsFailed: 'Karten konnten nicht geladen werden.',
    cardsCount: '{{count}} Karten',
    ownedTag: 'Besitze ich',
    addMissingButton: '{{count}} fehlende Karten auf Wunschliste',
    wishlistAdded: 'Fehlende Karten wurden zur Wunschliste hinzugefügt.',
    wishlistFailed: 'Wunschliste konnte nicht aktualisiert werden.',
  },
  settings: {
    title: 'Einstellungen',
    languageTitle: 'Sprache',
    geminiTitle: 'Gemini API Key',
    geminiDescription: 'Für die KI-gestützte Kartenerkennung beim Scannen.',
    manageGeminiKey: 'Gemini API Key verwalten',
    bulkDataTitle: 'Kartendatenbank',
    bulkDataDescription: 'Lokale Kopie aller Karten für Scanner und Sammlung - hilft, wenn Kartendaten wegen einer wackligen Verbindung nicht laden.',
    bulkDataReady: '{{count}} Karten lokal gespeichert.',
    bulkDataNotReady: 'Noch keine lokale Kartendatenbank vorhanden.',
    bulkDataReloadSuccess: 'Kartendatenbank erfolgreich aktualisiert.',
    reloadBulkData: 'Jetzt neu laden',
  },
  geminiKey: {
    backToSettings: '← Zurück zu den Einstellungen',
    title: 'Gemini API Key',
    hint: 'Für die KI-gestützte Kartenerkennung beim Scannen. Kostenlos unter',
    keyLabel: 'Gemini API Key',
    keyPlaceholder: 'AIzaSy... oder AQ.Ab...',
    keySet: '✓ Key hinterlegt',
    keyUnset: 'Noch kein Key hinterlegt',
    save: 'Speichern',
    saving: 'Speichert…',
    testKey: 'Key testen',
    testing: 'Testet…',
    saveSuccess: '✓ Key gespeichert',
    saveFailed: 'Key konnte nicht gespeichert werden.',
    testValid: '✓ Key ist gültig',
    testInvalid: 'Key ist ungültig oder nicht erreichbar',
  },
  profile: {
    title: 'Profil',
    pictureTitle: 'Profilbild',
    uploadPicture: 'Bild hochladen',
    uploading: 'Lädt hoch…',
    avatarInvalidType: 'Bitte eine Bilddatei auswählen.',
    avatarTooLarge: 'Bild ist zu groß (max. 5 MB).',
    avatarUploadFailed: 'Bild konnte nicht hochgeladen werden.',
    nameTitle: 'Anzeigename',
    nameLabel: 'Name',
    save: 'Speichern',
    saving: 'Speichert…',
    nameSaved: '✓ Name gespeichert',
    saveFailed: 'Name konnte nicht gespeichert werden.',
    emailTitle: 'E-Mail',
    viaGoogle: 'via Google',
  },
  userMenu: {
    profile: 'Profil',
    settings: 'Einstellungen',
    geminiKey: 'Gemini API Key',
  },
  geminiOnboarding: {
    title: 'KI-Kartenerkennung einrichten',
    message: 'Für den vollen Scanner-Umfang benötigst du einen kostenlosen Gemini API Key.',
    setupNow: 'Jetzt einrichten',
    later: 'Später',
  },
};
